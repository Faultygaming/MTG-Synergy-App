/**
 * Force-directed variant of preview-map-stress. Renders the synthetic
 * 99-card deck through packForce + edge-tier-weighted springs.
 *
 *   pnpm tsx scripts/preview-map-stress-force.ts
 *
 * Output: data/map-preview-stress-force.svg
 */
...
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" font-family="ui-sans-serif, system-ui, sans-serif">
  ...
  <g font-size="13" fill="#e7e5e4">
    <text x="20" y="${H - 42}" font-weight="600">FORCE · synthetic Hearthhull-shaped deck</text>
    <text x="20" y="${H - 24}" font-size="11" fill="#a1a1aa">Tier-weighted springs; ${edges.length} edges, ${nodes.length} cards</text>
  </g>
</svg>`;
  const outDir = resolve(process.cwd(), "data");
  mkdirSync(outDir, { recursive: true });
  const out = resolve(outDir, "map-preview-stress-force.svg");
